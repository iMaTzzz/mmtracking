...
class SiamRPN(BaseSingleObjectTracker):
    ...
    def get_cropped_img(self, img, center_xy, target_size, crop_size,
        ...
        _left_pad = torch.max(-context_xmin, torch.tensor(0, device=context_xmin.device))
        _top_pad = torch.max(-context_ymin, torch.tensor(0, device=context_ymin.device))
        _right_pad = torch.max(context_xmax - W, torch.tensor(0, device=context_xmax.device))
        _bottom_pad = torch.max(context_ymax - H, torch.tensor(0, device=context_ymax.device))
